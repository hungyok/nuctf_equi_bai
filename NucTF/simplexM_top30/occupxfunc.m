 ...
 if invld==1
    ...
    for chr=1:chrindx
        Lgnome=length(E{1,chrlist(chr)});
        m=mod(Lgnome-3*dLb,dLb);
        if m==0
           pLm=1+(Lgnome-3*dLb)/dLb; EN=E{1,chrlist(chr)}; ETF=Etfmul{1,chrlist(chr)};
        else
           pLm=2+(Lgnome-3*dLb-m)/dLb;
           EN=cat(1,E{1,chrlist(chr)},(zeros(dLb-m,1)+Em));
           ETF=cat(1,Etfmul{1,chrlist(chr)},E2(1:dLb-m,1:tfn));
        end
        ...
        ON=nor*O;
        ONLx{chrlist(chr),1}=ON(x1_lee{chrlist(chr),1}(100:end-100));
        %fprintf('chr %d pLm %d \n',chr,pLm); 
    end
    ...
    for chr=1:chrindx
        if chr==1
           ONL=ONLx{chrlist(chr),1}; yL=y1_lee{chrlist(chr),1}(100:end-100);
        else
           ONL=cat(1,ONL,ONLx{chrlist(chr),1}); yL=cat(1,yL,y1_lee{chrlist(chr),1}(100:end-100)); 
        end
    end
    Oer=ONL-yL;
