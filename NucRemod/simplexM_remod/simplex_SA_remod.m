...
fnxbak = fullfile(strcat(path4,foldername1),'simplex_xvalbak.txt');
fnf = fullfile(strcat(path4,foldername1),'simplex_fval.txt');
fnt = fullfile(strcat(path4,foldername1),'simplex_tval.txt');
fnT = fullfile(strcat(path4,foldername1),'simplex_Temp.txt');
fileID = fopen(fnT,'w'); 
...
xi=x;
for i=1:(n+1) 
    Fsig(i,1)=occupRfunc(xi(:,i),tfx,path1,path2,path3,path4); fprintf('print i....%d \n',i); 
end
           ...
           while flgx1>0
                 if k<=4 % k<=n for low n
                    ...
                    for ki=(n-k+2):(n+1)
                        ...
                        xi=xr(:,ki);
                        Fsigxr(ki,1)=occupRfunc(xi,tfx,path1,path2,path3,path4);
                    end
                    ...
                    xi=x(:,2:n+1);
                    for i=2:n+1  
                        Fsig(i,1)=occupRfunc(xi(:,i-1),tfx,path1,path2,path3,path4);
                    end
