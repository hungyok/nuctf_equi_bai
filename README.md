...
The SEM partition function is a summation of Boltzmann weight: c<sub>t/N</sub>e<sup>-γ<sub>t/N</sub>E<sup>i</sup><sub>t/N</sub></sup> where E<sup>i</sup><sub>t/N</sub></sup> stands for the binding energy of nucleosome (N) or TF (t) at a genomic loaction i. The scaling factors c<sub>t/N</sub> and γ<sub>t/N</sub> are the free parameters of the model. We use a modified Nelder-Mead simplex algorithm with Simulated Annealing to optimize c and γ by minimizing the root-square-mean deviation (RMSD) of nucleosome occupancy between the modified expiremental data (yy3A_lee.mat) and our model.
